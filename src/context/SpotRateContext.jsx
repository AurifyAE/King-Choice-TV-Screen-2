...
export const SpotRateProvider = ({ children }) => {
  ...
  const calculateValues = useCallback((bid = 0, bidSpread = 0, askSpread = 0, offset = 0, precision = 2) => {
  ...
  const updateMarketData = useCallback(
    (marketData, goldBidSpread, goldAskSpread, silverBidSpread, silverAskSpread) => {
      if (marketData) {
        ...
        const silverValues = calculateValues(marketData.Silver?.bid, silverBidSpread, silverAskSpread, 0.05, 2);
        setSilverData({
    ...
    [calculateValues] // stable dependency
  );
